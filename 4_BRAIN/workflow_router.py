...
if __name__ == "__main__":
    ...
    if mode_override:
        # Force mode but still detect file paths/URLs to parse them properly
        _, processed_input = detect_input_type(input_val)
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
        sys.path.append(project_root)
        from importlib import import_module
        pipeline = import_module('4_BRAIN.pipeline_manager')
        pipeline.run_pipeline(processed_input, brand=brand, mode=mode_override, aspect_ratio=ratio, project_name=name)
    else:
