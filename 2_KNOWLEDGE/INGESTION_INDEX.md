...
| **koharu** (mayocream) | B | onscreen-translate | **ingested** | **GPL-3.0** (isolate as separate process). Fills the on-screen/hardsub TEXT translation axis (detect→OCR→translate→**inpaint**→re-render) that audio dub can't — title cards, hardsubs, thumbnails. 2_KNOWLEDGE/repos/koharu-onscreen-translate.md. Pairs with localizer (audio) + local Gemma. |
