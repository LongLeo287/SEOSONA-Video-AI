...
| **VideoLingo (Huanshere)** | A | subtitle/dub | **ingested (pattern)** | Apache-2.0. Its WhisperX single-line semantic subtitle cutting is implemented as `group_words_to_segments` (srt_maker/whisper_engine.py) and used by `_extract_srt_from_media`. Translate→dub stage = future repurposer upgrade (rewire to PhoWhisper + VieNeu). |
