...
- ✅ **Shader transitions WORK** — a 3-scene project with `whip-pan` + `light-leak` rendered successfully. The IIFE is vendored at `.agents/skills/graphic-overlays/assets/vendor/shader-transitions.global.js` (the old `5_FRAMEWORK/hf_core/` reference HTML was removed when hf_core was decommissioned — hf_engine is the live registry now).
